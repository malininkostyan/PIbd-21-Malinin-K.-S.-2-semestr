...
{
    public class LogServiceDB : ILogService
    {
        private AbstractContextDb context;
        public LogServiceDB(AbstractContextDb context)
        {
        ...
        public void SaveBouquetPrice(LogBindingModel model)
        {
            ...
            try
            {
                ...
                var bouquets = context.Bouquets.ToList();
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, bouquets.Count, 2, ref
                missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = bouquets[i].BouquetName;
                    table.Cell(i + 1, 2).Range.Text = bouquets[i].Cost.ToString();
                }
        ...
        public List<LoadStoragesViewModel> GetStoragesLoad()
        {
            return context.Storages
            .ToList()
            .GroupJoin(
            context.StorageElements
            .Include(r => r.Element)
            .ToList(),
            storage => storage,
            storageElement => storageElement.Storage,
            (storage, storageCompList) => new LoadStoragesViewModel
            {
                StorageName = storage.StorageName,
                TotalCount = storageCompList.Sum(r => r.Amount),
                Elements = storageCompList.Select(r => new Tuple<string,
                int>(r.Element.ElementName, r.Amount))
            })
            .ToList();
        }
        ...
        public List<CustomerBookingViewModel> GetCustomerBookings(LogBindingModel model)
        {
            return context.Bookings
            .Include(rec => rec.Customer)
           .Include(rec => rec.Bouquet)
           .Where(rec => rec.CreateDate >= model.DateFrom &&
           rec.CreateDate <= model.DateTo)
            .Select(rec => new CustomerBookingViewModel
            {
                CustomerName = rec.Customer.CustomerFIO,
                CreateDate = SqlFunctions.DateName("dd", rec.CreateDate) + " " +   SqlFunctions.DateName("mm", rec.CreateDate) 
        ...
        public void SaveCustomerBookings(LogBindingModel model)
        {
            ...
            table.AddCell(new PdfPCell(new Phrase("Букет", fontForCellBold))
            {
                HorizontalAlignment = Element.ALIGN_CENTER
            });
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CreateDate, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].BouquetName, fontForCells));
                table.AddCell(cell);
